...
std::vector<double> cuRidgeSolve(std::vector<double> const &rhs_matrix, std::vector<double> const &lhs_vector, int const m, int const n, double const lamda)
{
    ...
    int const ldb = n;
    int const ldc = n;
    CUBLAS_CHECK(cublasDgemm(cublasH, transpose, no_transpose, n, n, m, &alpha, d_rhs_matrix, m, d_rhs_matrix, m, &beta, d_rhs_t_rhs_prod, ldc));
    ...
    int const incx = 1;
    int const incy = 1;
    CUBLAS_CHECK(cublasDgemv(cublasH, transpose, m, n, &alpha, d_rhs_matrix, m, d_lhs_vector, incx, &beta, d_rhs_t_lhs_prod, incy));
